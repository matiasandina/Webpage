...
# Student A knows 40 % of the knowledge and will get to 80% (dK = 40%)

studentA <- 0.4 * mytime - rnorm(1, mean = 0, sd= 10) + 40
studentB <- 0.4 * mytime - rnorm(1, mean = 0, sd= 10) + 10

# Student that pass but does not learn much (change ~ 0)
...
para_progress <- melt(data.frame(time=mytime, studentA=studentA, studentB=studentB), id.vars = "time")
...
para_progress %>% ggplot(aes(time, knowledge)) +
  geom_point(aes(color=student), size=2, alpha=0.8) + 
  scale_color_manual(values = c("#44AA99", "#DDAA77")) +
  theme(legend.position="bottom")+
  geom_hline(yintercept = 60, lty=2) +
  ylab("Cumulative Knowledge (%)") + ylim(c(0,100))+
  ggtitle("Student Progress")+
  geom_text(data = data.frame(x=5, y=c(55,65), label=c("Fail", "Pass")), aes(x,y,label=label), inherit.aes=FALSE)
...
table_1 <- para_progress %>% group_by(student) %>% summarise(max= round(max(knowledge),2), min=round(min(knowledge),2), change = max(knowledge)-min(knowledge))

pander::pander(table_1)
...
```{r inter_progress, echo=F}

# Data frame with interaction

inter_progress <- melt(data.frame(time=mytime, studentB=studentB, studentC=studentC), id.vars = "time")
...
inter_progress %>% ggplot(aes(time, knowledge)) +
  geom_point(aes(color=student), size=2, alpha=0.8) + 
...
table_2 <- inter_progress %>% group_by(student) %>% summarise(max= round(max(knowledge),2), min=round(min(knowledge),2), change = round(max(knowledge)-min(knowledge),2))

pander::pander(table_2)
...
```{r path_plot_1, echo=F}

para_progress %>% ggplot(aes(time, knowledge)) +
#  geom_point() +
  geom_step(aes(color=student), lwd=1.2) +
  scale_color_manual(values = c("#44AA99", "#DDAA77")) +
#  annotate("rect", xmin=38, xmax=50, ymin=-Inf, ymax=Inf,
#                             fill="magenta",alpha=0.1)+
  theme(legend.position="bottom")+
  geom_hline(yintercept = 60, lty=2) +
  ylab("Cumulative Knowledge (%)") + ylim(c(0,100))+
  ggtitle("Student Progress")+
...
fast_progress <- melt(data.frame(time=mytime, Faster=Faster, Slower=Slower), id.vars = "time")
...
fast_progress %>% ggplot(aes(time, knowledge)) +
  ...
  ylab("Cumulative Knowledge (%)") + ylim(c(0,110))+
  ggtitle("Student Progress")+
  geom_text(data = data.frame(x=15, y=c(95,105), label=c("In-Progress", "Master")), aes(x,y,label=label), inherit.aes=FALSE)+
  geom_text(data = data.frame(x=27, y=c(105), label="*"), aes(x,y,label=label), size=8, inherit.aes=FALSE)
