...
```{r read-into-R}
df <- read.csv("data_for_posts/gmail_mbox.csv",
...
```{r sourcing-github, message=FALSE}
...
MIT_sent <- read.csv('data_for_posts/MIT_Sent.csv',
                     header=FALSE, stringsAsFactors = FALSE)
names(MIT_sent) <- c('subject', 'from', 'date', 'to', 'label')
MIT_sent$label <- "Sent"

MIT_received <- read.csv("data_for_posts/MIT_inbox.csv", header=FALSE,
                         stringsAsFactors = FALSE)
names(MIT_received) <- c('subject', 'from', 'date', 'to', 'label')
MIT_received$label <- "Received"

hotmail_sent <- read.csv('data_for_posts/hotmail_Sent.csv', header=FALSE,
                         stringsAsFactors = FALSE)
names(hotmail_sent) <- c('subject', 'from', 'date', 'to', 'label')
hotmail_sent$label <- "Sent"

hotmail_received <- read.csv("data_for_posts/hotmail_inbox.csv", header=FALSE,
                             stringsAsFactors = FALSE)
names(hotmail_received) <- c('subject', 'from', 'date', 'to', 'label')
hotmail_received$label <- "Received"

umass_sent <- read.csv('data_for_posts/umass_Sent.csv', header=FALSE,
                       stringsAsFactors = FALSE)
names(umass_sent) <- c('subject', 'from', 'date', 'to', 'label')
umass_sent$label <- "Sent"

umass_received <- read.csv("data_for_posts/umass_inbox.csv", header=FALSE,
                           stringsAsFactors = FALSE)
...
emails <- emails %>% mutate(account = "Gmail") %>%
  ...
  mutate(simple_label = ifelse(str_detect(label, pattern = "Recibido|Received"),
                               "Received", "Sent"))
...
```{r modify-dates, message=FALSE, warning=FALSE}
...
```{r tile-plot, echo=FALSE, height=6}
# Make summary
tile_data <- emails %>% group_by(my_year, my_month, my_day) %>%
             count() %>%
             ungroup() %>% group_by(my_year, my_month) %>%
             summarise(month_count = sum(n))

    ggplot(tile_data, aes(my_month, my_year, fill=month_count)) +
...
emails %>%
  ...
  #annotate("segment", linetype = 1, size = 0.25, x=2014.5, xend = 2012,
  #           y= 1800, yend = 2800,
  #           arrow = arrow(ends = "first", length = unit(0.3, "line")))+
  annotate("text", label=c("ARG","US"), x=c(2015, 2016.8), y=-1)+
  labs(x="", y="Number of emails", color="")+
...
lala_sum %>% ggplot(aes(my_year, mean_n))+
  geom_point(size=3)+
  geom_segment(aes(x=my_year, xend=my_year, y=min_n, yend=max_n))+
  coord_flip()+
  theme_bw()+
  ggtitle("Mean emails per day",
          "Segment shows the range")+
  xlab("") + ylab("")
...
ggplot(data.frame(distance=distance),
       aes(x=1:length(distance), distance)) + geom_line() +
  theme_bw() +
...
```{r answer, echo=FALSE, comment=NA}
