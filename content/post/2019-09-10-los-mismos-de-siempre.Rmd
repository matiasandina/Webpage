...
diputados_file <- list.files("data_for_posts/", pattern = "diputados",full.names = TRUE)
...
senadores_file <- diputados_file <- list.files("data_for_posts/", pattern = "senadores",full.names = TRUE)
...
Los senadores son representantes según su provincia. Uno esperaría que tengan fuerte arraigo y sean portavoces de las necesidades particulares de aquellos a los que representan. Esto, en gran medida, es cierto. Los  
...
```{r obtener-total-partidos, echo=FALSE}
...
## analisis de partidos politicos

Cuales son las palabras más comunes de los partidos politicos
...
## Analizar de que partido a que partido fueron
...
```{r show-code, ref.label=all_labels(!exists('engine')), echo = TRUE, eval=FALSE}
